package yc.com.ApiCase;

import java.util.Random;

import javax.naming.NameNotFoundException;

public class ZYNameHelper {
	public static void main(String[] args) {
		ZYNameHelper QM = new ZYNameHelper();
		QM.createName("张", true);
		QM.createName("李");
		...
		QM.createName("王", true, 1);
		QM.createName("刘", true, 2);
		QM.createName("李", true, 3);
		QM.createName("赵", false, 1);
		QM.createName("贾", false, 2);
		QM.createName("孙", false, 3);
	...
	// 常用男性名数组
	public static final String[] MAN_XINGS = { "嘉懿", "煜城", "懿轩", "烨伟", "苑博", "伟泽", "熠彤", "鸿煊", "博涛", "烨霖", "烨华", "煜祺",
	...
	// 常用女性名数组
	public static final String[] WOMAN_XINGS = { "梦琪", "忆柳", "之桃", "慕青", "问兰", "尔岚", "元香", "初夏", "沛菡", "傲珊", "曼文", "乐菱",
	 ...
	 * @return
	 */
	public String createName(String xing, boolean isMan) {
		String ming = null;
		Random rd = new Random();
		String name;
		if (true == isMan) {
			ming = MAN_XINGS[rd.nextInt(20)];
			name = xing + ming;
			System.out.println("姓名:" + name + " " + "性别:" + "男");
		} else {
			ming = WOMAN_XINGS[rd.nextInt(20)];
			name = xing + ming;
	 ...
	 * @return
	 */
	public String createName(String xing) {
		String ming = null;
		Random rd = new Random();
		String name = xing + MAN_XINGS[rd.nextInt(30)];
	 ...
	 * 
	 * @param isMan
	 *            true 表示男性, false 表示女性
	 * @return
	 */
	public static String createName(boolean isMan) {
		Random rd = new Random();
		String name;
		String xing = XINGS[rd.nextInt(20)];
		if (true == isMan) {
			name = xing + MAN_XINGS[rd.nextInt(20)];
			System.out.println("该男性姓名为:" + name);
		} else {
			name = xing + WOMAN_XINGS[rd.nextInt(20)];
			System.out.println("该女性姓名为:" + name);
		}
		return name;
	}
	...
	public static String createName() {
		Random rd = new Random();
		String name = XINGS[rd.nextInt(30)] + MAN_XINGS[rd.nextInt(20)];
		System.out.println("随机性名:" + name);
		return name;
	}
	 ...
	 *            是否是男性
	 * @param j
	 *            名的字数, 1~3 个字, 超过3个抛出 NameException 异常 例如：赵建国：2，王勇：1
	 * @return
	 */
	public static String createName(String xing, boolean isMan, int j) {
		String name;
		Random rd = new Random();
		int i=rd.nextInt(30);
		String ming;
		if (true == isMan) {
			try {
				if (j > 3) {
					throw new NameException("字数太多,无法生成");
				}
				if (j == 1) {
					ming = MAN_STRING.substring(i, i+1);
					name = xing + ming;
					System.out.println(name + ":" + j);
				}
				if(j==2){
					ming = MAN_STRING.substring(i, i+2);
					name = xing + ming;
					System.out.println(name + ":" + j);
				}
			} catch (Exception e) {
				e.printStackTrace();
				System.out.println("名异常");
			}
		} else {
			try {
				if (j > 3) {
				...
				if (j == 1) {
					ming = WOMAN_STRING.substring(i, i+1);
					name = xing + ming;
					System.out.println(name + ":" + j);
				}
				if(j==2){
					ming = WOMAN_STRING.substring(i, i+2);
					name = xing + ming;
					System.out.println(name + ":" + j);
				}
			} catch (Exception e) {
